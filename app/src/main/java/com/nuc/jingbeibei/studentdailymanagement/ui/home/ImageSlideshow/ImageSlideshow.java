package com.nuc.jingbeibei.studentdailymanagement.ui.home.ImageSlideshow;
...
public class ImageSlideshow extends FrameLayout {
    ...
    private void initView() {
        contentView = LayoutInflater.from(context).inflate(R.layout.is_banner_layout, this, true);
    ...
    private void setViewList(List<ImageTitleBean> imageTitleBeanList) {
        ...
        for (int i = 0; i < count + 2; i++) {
            View view = LayoutInflater.from(context).inflate(R.layout.is_banner_title_layout, null);
...
//                        load(imageTitleBeanList.get(count - 1).getImageUrl()).into(ivImage);
                Picasso.with(context)
                        .load(imageTitleBeanList.get(count - 1).getImageUrl())
